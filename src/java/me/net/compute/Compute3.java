...
public class Compute3 extends Compute {
	...
	public void buyOrSell(String hcode, StockDay someDay, 
		...
		eStockOper result = simulator3.handle(someDay, operationFunction);
	...
	private void buy2(String hcode, StockDay nextDay, List<OperRecord> operList) {
		...
		if(!checkConfig(date)) return;
		
		if(this.check(operList, price, date) == false)
			return;

		// 以固定总价来买, 取一个整的数量，然后以这个数量来买。卖还是以量来卖 
		int num = (int)(oneAmount/price.doubleValue());
		if(strategy == eStrategy.Ratio)
			num = ratioStrategy(num, operList, price);
	...
	private void sell2(String hcode, StockDay someDay, StockDay nextDay, 
		...
		if (!isPractice && last.getDate_() != null && last.getDate_().equals(someDay.date_)) { //昨天决定今天买，今天不能卖
		...
		if(nextDay == null) { //最后一天全卖
			...
			num = lastTotal;
		} else if (gain_high.doubleValue() > cost.doubleValue() * sellRatio_win) {
			num = lastTotal;
			sum = new BigDecimal(cost.doubleValue() * sellRatio_win);
		} else if (gain_low.doubleValue() < cost.doubleValue() * sellRatio_lose) {
			num = lastTotal;
			sum = new BigDecimal(cost.doubleValue() * sellRatio_lose);
	...
	private boolean checkConfig(Date date) {
		try {
			if (startBuyDate == null) {
				logger.error("startBuyDate is null");
				return false;
			}
			if(date.before(format.parse(startBuyDate)))
				return false;
	...
	//买多少数量的策略。1.
	private int ratioStrategy(int num, List<OperRecord> operList, BigDecimal price) {
		...
		while(size > 0) {
			if (size > 0 && operList.get(size - 1).getOper().equals(eStockOper.Buy.toString())
					&& tmpPrice.doubleValue() < operList.get(size - 1).getPrice().doubleValue()) {
				tmpPrice = operList.get(size - 1).getPrice();
				size--;
				num = (int) ((int)num * buyNumRatio);
			} else 
				break;
		}
